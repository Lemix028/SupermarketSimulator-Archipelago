...
namespace SupermarketArchipelago
{
    ...
    public static class MainMenuPatch
    {
        ...
        private static float _connectingTimeout = 0f;
        ...
        public static void Postfix(MainMenuManager __instance)
        {
            ...
            if (rect != null) rect.anchoredPosition = new Vector2(rect.anchoredPosition.x + 425f, rect.anchoredPosition.y - 30f);
        ...
        [HarmonyPostfix]
        public static void EventSystemUpdatePostfix()
        {
            if (_apButton == null || _apButtonText == null) return;

            if (_connectingTimeout > 0f && !ArchipelagoClient.IsConnected)
            {
                _connectingTimeout -= Time.deltaTime;
                if (_connectingTimeout <= 0f)
                {
                    if (_apButtonText != null) _apButtonText.text = "Connect AP Server";
                }
            }
        ...
        public static void RefreshButtonText()
        {
            ...
            if (ArchipelagoClient.IsConnected)
            {
                _apButtonText.text = "Disconnect AP";
