...
namespace SupermarketArchipelago
{
    ...
    public static class ArchipelagoIdHelper
    {
        ...
        public const int BakerBaseID = 900;       
        public const int IceCreamHelperBaseID = 910;       
        public const int VendingMachineBaseID = 920;       
        ...
        private static readonly Dictionary<int, int> ApToGameFurniture = new Dictionary<int, int>
        {
            ...
            { 429, 312 },  // Trash Can
            { 430, 224 },  // Metal Hanger
            { 431, 225 },  // Wooden Hanger
            { 432, 226 },  // Wooden Pegboard Display
            { 433, 227 },  // Metal Pegboard Display
            { 434, 228 },  // Oven
            { 435, 229 },  // Single Bakery Shelf
            { 436, 230 },  // Bakery Shelf
            { 437, 231 },  // Ice Cream Stand
        };
        ...
        public static ApItemType GetItemType(long apID)
        {
            ...
            if (apID >= BakerBaseID && apID < 910) return ApItemType.Baker;
            if (apID >= IceCreamHelperBaseID && apID < 920) return ApItemType.IceCreamHelper;
            if (apID >= VendingMachineBaseID && apID < 921) return ApItemType.VendingSlot;
        ...
        public static int ToGameID(long apID)
        {
            ...
            if (apID >= BakerBaseID && apID < 910) return (int)(apID - BakerBaseID);
            if (apID >= IceCreamHelperBaseID && apID < 920) return (int)(apID - IceCreamHelperBaseID);
            if (apID >= VehicleBaseID && apID < 400) return (int)(apID - VehicleBaseID);
            if (apID >= VendingMachineBaseID && apID < 921) return (int)(apID - VendingMachineBaseID);
