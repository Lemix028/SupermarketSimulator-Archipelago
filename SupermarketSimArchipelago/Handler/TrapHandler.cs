...
namespace SupermarketArchipelago
{
    ...
    public static class TrapHandler
    {
        ...
        private static void ExecuteDustStorm()
        {
            try
            {
                ...
                if (allDustPoints != null && allDustPoints.Length > 0)
                {
                    foreach (var dust in allDustPoints)
                    {
                        if (dust != null)
                        {
                            int intensity = UnityEngine.Random.RandomRangeInt(5, 10);
            ...
            {
                Plugin.Log.LogError($"Error at Trash Flood Trap: {ex.Message}");
            }
        }
        ...
        {
            UIHelper.TriggerPowerOutageEvent(30f); // Maybe randomize duration in the future
        }
